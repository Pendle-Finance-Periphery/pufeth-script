...
export async function readAllUserActiveBalances(ctx: EthContext, allAddresses: string[]): Promise<bigint[]> {
  const multicall = getMulticallContractOnContext(ctx, MISC_CONSTS.MULTICALL);
  const market = getPendleMarketContractOnContext(ctx, PENDLE_POOL_ADDRESSES.LP);
